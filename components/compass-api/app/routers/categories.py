...
@router.post(
    "/", response_model=StandardResponse[Category], status_code=status.HTTP_201_CREATED
)
...
async def get_categories(
    ...
    limit: int = 100,
    sort: str = Query(
        "radar_quadrant", description="Sort field (prefix with - for descending order)"
    ),
    category_service: CategoryService = Depends(),
    ...
    return StandardResponse.paginated(
        data=categories_with_usage, total=total, skip=skip, limit=limit
    )


@router.get("/{category_id}", response_model=StandardResponse[Category])
async def get_category(
    category_id: str, category_service: CategoryService = Depends()
) -> StandardResponse[Category]:
...
async def update_category(
    ...
    try:
        category = await category_service.update_category_by_id(
            category_id, category_update, current_user.username
        )
        if not category:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Category not found"
