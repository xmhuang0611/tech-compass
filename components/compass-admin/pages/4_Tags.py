...
# Page configuration
st.set_page_config(page_title="Tags - Tech Compass Admin", page_icon="🏷️", layout="wide")
...
def confirm_delete(tag_data):
    ...
    st.warning(
        "This action cannot be undone! This will remove the tag from all solutions using it."
    )
...
def render_tag_form(tag_data):
    ...
    with st.form("edit_tag_form"):
        ...
        description = st.text_area(
            "Description", value=tag_data.get("description", ""), help="Tag description"
        )
        ...
        if submitted:
            ...
            if update_tag(tag_data["name"], update_data):
                ...
                st.error(
                    f"❌ Failed to update tag: {st.session_state.show_error_message}"
                )
                st.session_state.show_error_message = None
