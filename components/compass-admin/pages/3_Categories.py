...
# Page configuration
st.set_page_config(
    page_title="Categories - Tech Compass Admin", page_icon="📑", layout="wide"
)
...
def confirm_delete(category_data):
    ...
    if st.button("Yes, Delete", type="primary"):
        if delete_category(category_data["name"]):
            ...
            st.error(
                f"Failed to delete category: {st.session_state.show_error_message}"
            )
            st.session_state.show_error_message = None
...
def render_category_form(category_data):
    ...
    with st.form("edit_category_form"):
        ...
        # Basic Information
        name = st.text_input(
            "Name*", value=category_data.get("name", ""), help="Category name"
        )
        ...
        if submitted:
            ...
            if update_category(category_data["name"], update_data):
                ...
                st.error(
                    f"❌ Failed to update category: {st.session_state.show_error_message}"
                )
                st.session_state.show_error_message = None
...
def render_add_category_form():
    ...
    with st.form("add_category_form"):
        ...
        if st.session_state.show_error_message:
            st.error(
                f"❌ Failed to add category: {st.session_state.show_error_message}"
            )
            st.session_state.show_error_message = None
