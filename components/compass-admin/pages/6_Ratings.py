...
def load_ratings(solution_slug=None, skip=0, limit=20):
    ...
    try:
        ...
        if response and response.get("success"):
            ...
            st.error(f"Failed to load ratings: {response.get('detail', 'Unknown error occurred')}")
    except Exception as e:
...
def render_rating_details(rating_data):
    ...
    with col1:
        st.text_input("Solution", value=rating_data.get("solution_slug", ""), disabled=True)
        st.text_input("Username", value=rating_data.get("username", ""), disabled=True)
        st.text_input("Created At", value=rating_data.get("created_at", ""), disabled=True)
    with col2:
        ...
        st.text_input("Updated At", value=rating_data.get("updated_at", ""), disabled=True)
...
def main():
    ...
    if ratings:
        ...
        columns = [
            'id', 'solution_slug', 'score', 'comment', 'username', 'created_at', 'updated_at'
        ]
