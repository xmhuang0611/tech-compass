...
def load_comments(solution_slug=None, skip=0, limit=20):
    ...
    try:
        ...
        if response and response.get("status_code") == 200:
            ...
            st.error(f"Failed to load comments: {response.get('detail', 'Unknown error occurred')}")
    except Exception as e:
...
def render_comment_form(comment_data):
    ...
    with st.form("edit_comment_form"):
        ...
        with col1:
            st.text_input("Solution", value=comment_data.get("solution_slug", ""), disabled=True)
            st.text_input("Username", value=comment_data.get("username", ""), disabled=True)
            st.text_input("Created At", value=comment_data.get("created_at", ""), disabled=True)
        with col2:
            st.text_input("Comment ID", value=comment_data.get("id", ""), disabled=True)
            st.text_input("Updated At", value=comment_data.get("updated_at", ""), disabled=True)
        ...
        if st.session_state.show_error_message:
            st.error(f"❌ Failed to update comment: {st.session_state.show_error_message}")
            st.session_state.show_error_message = None
...
def main():
    ...
    if comments:
        ...
        columns = [
            'id', 'solution_slug', 'content', 'username', 'created_at', 'updated_at'
        ]
